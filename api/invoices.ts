      ...
      if (
        ...
        try {
          ...
          const response = await fetch(
            `${servemanagerConfig.baseUrl}/invoices`,
            {
              headers: {
          ...
          if (response.ok) {
            ...
            const transformedInvoices =
              data.data?.map((invoice: any) => ({
                id: invoice.id,
                invoice_number:
                  invoice.attributes?.invoice_number || `INV-${invoice.id}`,
                client_company:
                  invoice.attributes?.client_company || "Unknown Client",
                ...
                created_date:
                  invoice.attributes?.created_date || new Date().toISOString(),

    ...
    if (req.method === "GET") {
      ...
      if (servemanagerConfig.baseUrl && servemanagerConfig.apiKey) {
        try {
          ...
          console.log("🌐 VERCEL DEBUG - Fetching servers from ServeManager...");
          const response = await fetch(`${servemanagerConfig.baseUrl}/employees`, {
            headers: {
          ...
          if (response.ok) {
            ...
            const servers = data.data?.map((employee: any) => ({
              id: employee.id,
            ...
            console.log("❌ VERCEL DEBUG - ServeManager servers API error:", response.status);
          }
        } catch (error) {
          console.log("⚠️ VERCEL DEBUG - ServeManager servers request failed:", error);
        }
      }

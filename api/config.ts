...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const config = {
        serveManager: {
          ...
          apiKey: process.env.SERVEMANAGER_API_KEY ? '***' + process.env.SERVEMANAGER_API_KEY.slice(-4) : (tempConfig.serveManager?.apiKey ? '***' + tempConfig.serveManager.apiKey.slice(-4) : ''),
        ...
        stripe: {
          ...
          secretKey: process.env.STRIPE_SECRET_KEY ? '***' + process.env.STRIPE_SECRET_KEY.slice(-4) : (tempConfig.stripe?.secretKey ? '***' + tempConfig.stripe.secretKey.slice(-4) : ''),
          webhookSecret: process.env.STRIPE_WEBHOOK_SECRET ? '***' + process.env.STRIPE_WEBHOOK_SECRET.slice(-4) : (tempConfig.stripe?.webhookSecret ? '***' + tempConfig.stripe.webhookSecret.slice(-4) : ''),
        ...
        radar: {
          ...
          secretKey: process.env.RADAR_SECRET_KEY ? '***' + process.env.RADAR_SECRET_KEY.slice(-4) : (tempConfig.radar?.secretKey ? '***' + tempConfig.radar.secretKey.slice(-4) : ''),
    ...
    if (req.method === 'POST') {
      ...
      console.log('Config saved temporarily. For persistent storage, set these environment variables in Vercel:');
      ...
      return res.status(200).json({ 
        message: 'Configuration saved successfully for this session. Set environment variables in Vercel for persistence.' 
      });
    }

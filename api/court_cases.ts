    ...
    if (req.method === 'GET') {
      ...
      if (servemanagerConfig.baseUrl && servemanagerConfig.apiKey) {
        try {
          ...
          const response = await fetch(`${servemanagerConfig.baseUrl}/court_cases`, {
            headers: {
          ...
          if (response.ok) {
            ...
            const transformedCases = data.data?.map((courtCase: any) => ({
              id: courtCase.id,
              number: courtCase.attributes?.case_number || courtCase.attributes?.number || `CASE-${courtCase.id}`,
              plaintiff: courtCase.attributes?.plaintiff || 'Unknown Plaintiff',
              defendant: courtCase.attributes?.defendant || 'Unknown Defendant',
              court: {
                name: courtCase.attributes?.court_name || courtCase.attributes?.court?.name || 'Unknown Court',
                county: courtCase.attributes?.county || courtCase.attributes?.court?.county,
                state: courtCase.attributes?.state || courtCase.attributes?.court?.state || 'GA'
              },
              filed_date: courtCase.attributes?.filed_date || courtCase.attributes?.created_at,
              court_date: courtCase.attributes?.court_date || courtCase.attributes?.hearing_date,
              updated_at: courtCase.attributes?.updated_at || new Date().toISOString()
            })) || [];

            return res.status(200).json({ court_cases: transformedCases });
          }
        } catch (error) {
          console.log('ServeManager not available, using mock court cases:', error);
        }
      }

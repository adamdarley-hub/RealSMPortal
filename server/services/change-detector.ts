...
class ChangeDetector extends EventEmitter {
  ...
  private async detectChanges(jobIds: string[]) {
    console.log(
      "🔍 Serverless mode: Skipping change detection to prevent API spam",
    );
  ...
  private async _detectChanges_disabled(jobIds: string[]) {
    try {
      const { makeServeManagerRequest } = await import(
        "../routes/servemanager"
      );
      ...
      for (const [jobId, snapshot] of jobsToCheck) {
        try {
          ...
          const currentState = {
            ...
            lastAttemptId:
              freshJob.attempts?.[freshJob.attempts.length - 1]?.id,
          ...
          if (currentState.attemptCount > snapshot.attemptCount) {
            const newAttempts =
              freshJob.attempts?.slice(snapshot.attemptCount) || [];
            changes.push({
              jobId,
          ...
          if (currentState.status !== snapshot.status) {
            ...
            console.log(
              `📈 Status change for job ${jobId}: ${snapshot.status} → ${currentState.status}`,
            );
          }
          ...
          console.log(
            `⚠️ Could not check job ${jobId} for changes:`,
            error.message,
          );
        }
      }

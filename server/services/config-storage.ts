...
export class ConfigStorageService {
  async isAvailable(): Promise<boolean> {
    try {
      if (!isSupabaseConfigured()) {
        console.log(
          "💾 ConfigStorage: Supabase not configured, using fallback",
        );
        return false;
      }
      ...
      if (error) {
        console.log(
          "💾 ConfigStorage: Table not available, using fallback:",
          error.message,
        );
        return false;
      }
  ...
  async getServiceConfig(serviceName: string): Promise<StoredApiConfig | null> {
    try {
      ...
      console.error(
        `💾 ConfigStorage: Failed to get ${serviceName} config:`,
        error,
      );
      return null;
    }
  ...
  async getEffectiveConfig(): Promise<ApiConfigData> {
    ...
    result.serveManager = {
      baseUrl:
        ...
        storedConfig.serveManager?.baseUrl ||
        "",
      apiKey:
        process.env.SERVEMANAGER_API_KEY ||
        storedConfig.serveManager?.apiKey ||
        "",
      enabled: Boolean(
        process.env.SERVEMANAGER_BASE_URL ||
          process.env.SERVEMANAGER_API_KEY ||
    ...
    result.stripe = {
      publishableKey:
        ...
        storedConfig.stripe?.publishableKey ||
        "",
      secretKey:
        process.env.STRIPE_SECRET_KEY || storedConfig.stripe?.secretKey || "",
      webhookSecret:
        process.env.STRIPE_WEBHOOK_SECRET ||
        storedConfig.stripe?.webhookSecret ||
        "",
      enabled: Boolean(
        process.env.STRIPE_SECRET_KEY || storedConfig.stripe?.enabled,
      ),
      environment:
        (process.env.STRIPE_ENVIRONMENT as "test" | "live") ||
        storedConfig.stripe?.environment ||
        "test",
    };
    ...
    result.radar = {
      publishableKey:
        ...
        storedConfig.radar?.publishableKey ||
        "",
      secretKey:
        process.env.RADAR_SECRET_KEY || storedConfig.radar?.secretKey || "",
      enabled: Boolean(
        process.env.RADAR_SECRET_KEY || storedConfig.radar?.enabled,
      ),
      environment:
        (process.env.RADAR_ENVIRONMENT as "test" | "live") ||
        storedConfig.radar?.environment ||
        "test",
    };

...
export const getAttemptPhotoProxy: RequestHandler = async (req, res) => {
  try {
    ...
    if (!attempt || !attempt.misc_attachments) {
      return res.status(404).json({ error: 'Attempt or photos not found' });
    }

    const photo = attempt.misc_attachments.find((photo: any) => photo.id?.toString() === photoId);
    if (!photo || !photo.upload?.links?.download_url) {

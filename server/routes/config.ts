...
export async function loadConfig(): Promise<Partial<ApiConfig>> {
  try {
    ...
    try {
      ...
      if (
        config.radar?.secretKey &&
        !config.radar.secretKey.startsWith("***")
      ) {
        config.radar.secretKey = decrypt(config.radar.secretKey);
      }
      if (
        config.stripe?.secretKey &&
        !config.stripe.secretKey.startsWith("***")
      ) {
        config.stripe.secretKey = decrypt(config.stripe.secretKey);
      }
      if (
        config.stripe?.webhookSecret &&
        !config.stripe.webhookSecret.startsWith("***")
      ) {
      ...
      console.log(
        "Config file not found, checking environment variables and temp storage...",
      );
    }
    ...
    const config: Partial<ApiConfig> = {
      ...
      stripe: {
        publishableKey:
          ...
          "",
        secretKey:
          process.env.STRIPE_SECRET_KEY || tempConfig?.stripe?.secretKey || "",
        webhookSecret:
      ...
      radar: {
        publishableKey:
          ...
          "",
        secretKey:
          process.env.RADAR_SECRET_KEY || tempConfig?.radar?.secretKey || "",
        enabled: Boolean(
...
async function saveConfig(config: ApiConfig): Promise<void> {
  try {
    ...
    throw new Error(
      `Failed to process configuration: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
      ...
      serveManager: config.serveManager
        ? {
            ...
            apiKey: config.serveManager.apiKey
              ? "***" + config.serveManager.apiKey.slice(-4)
              : "",
          }
        : undefined,
      radar: config.radar
        ? {
            ...config.radar,
            secretKey: config.radar.secretKey
              ? "***" + config.radar.secretKey.slice(-4)
              : "",
          }
        : undefined,
      stripe: config.stripe
        ? {
            ...
            secretKey: config.stripe.secretKey
              ? "***" + config.stripe.secretKey.slice(-4)
              : "",
            webhookSecret: config.stripe.webhookSecret
              ? "***" + config.stripe.webhookSecret.slice(-4)
              : "",
          }
        : undefined,
    };
...
export const saveConfigHandler: RequestHandler = async (req, res) => {
  try {
    ...
    if (config.serveManager?.enabled && !config.serveManager.baseUrl) {
      return res
        .status(400)
        .json({ error: "ServeManager base URL is required when enabled" });
    }

    if (config.radar?.enabled && !config.radar.publishableKey) {
      return res
        .status(400)
        .json({ error: "radar.io publishable key is required when enabled" });
    }
...
export const testServeManager: RequestHandler = async (req, res) => {
  try {
    ...
    if (!baseUrl || !apiKey) {
      return res
        .status(400)
        .json({ error: "Base URL and API key are required" });
    }

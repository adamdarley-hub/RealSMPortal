...
export function useAutoSync(options: UseAutoSyncOptions = {}) {
    ...
    async (showLoading = true) => {
      ...
      try {
        ...
        if (isFullStoryPresent && status.consecutiveFailures > 0) {
          console.warn(
            "���� FullStory detected and causing fetch issues. Consider disabling auto-sync.",
          );
        }
        ...
        try {
          ...
          if (!healthCheck.ok && healthCheck.status >= 500) {
            console.warn(
              "⚠️ Server returned error status:",
              healthCheck.status,
            );
          ...
          if (healthError.name === "AbortError") {
            console.warn(
              "⏰ Health check timed out after 5 seconds, continuing with cached data",
            );
            if (mountedRef.current) {
              setStatus((prev) => ({
                ...prev,
          ...
          if (isNetworkError) {
            console.warn(
              "⚠️ Network connectivity issue, will retry with exponential backoff:",
              {
        ...
        if (error instanceof Error) {
          if (
            ...
            console.log(
              "�� Auto-sync request timed out after 30 seconds, using cached data",
            );
          } else if (
            error.message.includes("Failed to fetch") ||
            error.message.includes("Network connection failed")
            ...
            console.log(
              "🌐 Network error detected, will continue with cached data",
            );
        ...
        if (
          ...
          const backoffDelay = isNetworkError
            ? Math.min(interval * 2, 120000)
            : interval; // Max 2 minutes

          console.log(
            `��� Restarting auto-sync polling after sync error (delay: ${backoffDelay / 1000}s)`,
          );
        ...
        if (onDataUpdate && !showLoading) {
          console.log(
            "📦 Triggering data refresh with cached data due to sync error",
          );
          onDataUpdate();
        }
      }
  ...
  const startPolling = useCallback(() => {
    if (intervalRef.current || !enabled || !mountedRef.current) {
      console.log(
        "⏭️ Skipping start polling - already running or disabled or unmounted",
      );
      return;
    }
    ...
    if (status.consecutiveFailures >= 3) {
      ...
      if (isFullStoryPresent) {
        console.warn(
          "🔍 Auto-sync disabled due to FullStory interference. Too many consecutive failures.",
        );
        return;
      }
    }
    ...
    console.log(
      `🚀 Auto-sync started: every ${interval / 1000}s (first sync in 5s)`,
    );
